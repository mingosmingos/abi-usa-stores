################################################################################
# app3.R - Versão com GA O1
################################################################################

library(shiny)
library(shinythemes)
library(DT)
library(plotly)
library(dplyr)
library(GA)  # Para o Algoritmo Genético
library(mco)

source("hill.R")
source("eval_plan_O1.R")
source("eval_plan_O2.R")
source("eval_plan_O3.R")

# Constantes (IGUAIS AOS SCRIPTS)
MAX_J <- 50
MAX_X <- 30
MAX_PR <- 0.30
MAX_SALES_UNITS <- 10000

# Carregar previsões
raw <- read.csv("all_store_predictions.csv")
forecasts <- data.frame(
  Store    = raw$Store,
  Week_ID  = raw$Run,
  Day      = raw$Step,
  Forecast = raw$Num_Customers
)

semanas_disponiveis <- sort(unique(forecasts$Week_ID))
store_names <- c("baltimore", "lancaster", "philadelphia", "richmond")
store_names_print <- c("Baltimore", "Lancaster", "Philadelphia", "Richmond")

# ============================================================================
# FUNÇÕES EXISTENTES (HC, SANN, etc.)
# ============================================================================

# Solução inicial aleatória
random_solution <- function() {
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J, 28), rep(MAX_X, 28), rep(MAX_PR, 28))
  return(runif(84, min = lower, max = upper))
}

# Função de mudança
change_fn <- function(par, lower, upper) {
  hchange(par, lower = lower, upper = upper, operator = "*", 
          dist = rnorm, mean = 1, sd = 0.05, round = FALSE)
}

# Hill Climbing O1
run_optimization_O1 <- function(week_id, max_iter = 5000) {
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J, 28), rep(MAX_X, 28), rep(MAX_PR, 28))
  
  eval_fn <- function(sol) {
    eval_plan_O1(sol, forecasts = forecasts, week_id = week_id, verbose = FALSE)
  }
  
  set.seed(123)
  s0 <- random_solution()
  
  hc <- hclimbing(par = s0, fn = eval_fn, change = change_fn,
                  lower = lower, upper = upper, type = "max",
                  control = list(maxit = max_iter, REPORT = 0, digits = 2))
  
  return(list(solution = hc$sol, profit = hc$eval))
}

# Hill Climbing O2
run_optimization_O2 <- function(week_id, max_iter = 10000) {
  store_names <- c("baltimore", "lancaster", "philadelphia", "richmond")
  max_sales_units <- 10000
  
  max_J_dynamic <- numeric(28)
  max_X_dynamic <- numeric(28)
  
  idx <- 1
  for(s in 1:4) {
    fc_store <- forecasts[forecasts$Store == store_names[s] & forecasts$Week_ID == week_id, ]
    for(d in 1:7) {
      C_pred <- fc_store$Forecast[fc_store$Day == d]
      max_J_dynamic[idx] <- ceiling(C_pred * 1.5 / 6)
      max_X_dynamic[idx] <- ceiling(C_pred * 1.5 / 7)
      idx <- idx + 1
    }
  }
  
  max_J <- pmin(max_J_dynamic, 40)
  max_X <- pmin(max_X_dynamic, 25)
  max_PR <- rep(0.30, 28)
  
  lower <- rep(0, 84)
  upper <- c(max_J, max_X, max_PR)
  
  s0 <- array(0, dim = c(4, 7, 3))
  for(s in 1:4) {
    store <- store_names[s]
    fc_store <- forecasts[forecasts$Store == store & forecasts$Week_ID == week_id, ]
    for(d in 1:7) {
      C_pred <- fc_store$Forecast[d]
      X_heur <- floor(C_pred / 7)
      rest <- C_pred - 7 * X_heur
      J_heur <- ceiling(rest / 6)
      if(J_heur < 0) J_heur <- 0
      idx <- (s-1)*7 + d
      X_heur <- min(X_heur, max_X[idx])
      J_heur <- min(J_heur, max_J[idx])
      s0[s, d, 1] <- J_heur
      s0[s, d, 2] <- X_heur
      s0[s, d, 3] <- 0.15
    }
  }
  s0 <- c(s0)
  s0 <- repair_solution_inplace(s0, forecasts, week_id, max_sales_units)
  
  change_fn_explore <- function(par, lower, upper) {
    new_par <- par
    for(i in 1:length(par)) {
      if(runif(1) < 0.3) {
        if(par[i] == 0) {
          new_par[i] <- runif(1, 0, upper[i] * 0.2)
        } else {
          factor <- rnorm(1, mean = 1, sd = 0.1)
          new_par[i] <- par[i] * factor
        }
        new_par[i] <- min(upper[i], max(lower[i], new_par[i]))
      }
    }
    return(new_par)
  }
  
  eval_fn <- function(sol) {
    eval_plan_O2(sol, forecasts, week_id, max_sales_units, verbose = FALSE)
  }
  
  set.seed(123)
  hc <- hclimbing(par = s0, fn = eval_fn, change = change_fn_explore,
                  lower = lower, upper = upper, type = "max",
                  control = list(maxit = max_iter, REPORT = 0, digits = 2))
  
  final_profit <- eval_plan_O2(hc$sol, forecasts, week_id, max_sales_units, verbose = FALSE)
  return(list(solution = hc$sol, profit = final_profit))
}

# Hill Climbing O3
run_optimization_O3 <- function(week_id, max_iter = 8000, alpha = 0.1) {
  store_names <- c("baltimore", "lancaster", "philadelphia", "richmond")
  max_sales_units <- 10000
  
  max_J <- numeric(28)
  max_X <- numeric(28)
  idx <- 1
  for(s in 1:4) {
    fc_store <- forecasts[forecasts$Store == store_names[s] & forecasts$Week_ID == week_id, ]
    for(d in 1:7) {
      C_pred <- fc_store$Forecast[d]
      max_J[idx] <- min(ceiling(C_pred * 1.5 / 6), 40)
      max_X[idx] <- min(ceiling(C_pred * 1.5 / 7), 25)
      idx <- idx + 1
    }
  }
  
  max_PR <- rep(0.30, 28)
  lower <- rep(0, 84)
  upper <- c(max_J, max_X, max_PR)
  
  build_s0 <- function() {
    s0 <- array(0, dim = c(4, 7, 3))
    for(s in 1:4) {
      fc_store <- forecasts[forecasts$Store == store_names[s] & forecasts$Week_ID == week_id, ]
      for(d in 1:7) {
        C_pred <- fc_store$Forecast[d]
        X_heur <- floor(C_pred / 7)
        rest <- C_pred - 7 * X_heur
        J_heur <- max(0, ceiling(rest / 6))
        X_heur <- min(X_heur, max_X[(s-1)*7 + d])
        J_heur <- min(J_heur, max_J[(s-1)*7 + d])
        s0[s, d, 1] <- J_heur
        s0[s, d, 2] <- X_heur
        s0[s, d, 3] <- 0.15
      }
    }
    s0 <- repair_solution_inplace(c(s0), forecasts, week_id, max_sales_units, max_J, max_X)
    return(s0)
  }
  
  change_fn_mixed <- function(par, lower, upper) {
    new_par <- par
    for(i in 1:length(par)) {
      if(runif(1) < 0.3) {
        if(par[i] == 0) {
          new_par[i] <- runif(1, 0, upper[i] * 0.2)
        } else {
          new_par[i] <- par[i] * rnorm(1, mean = 1, sd = 0.1)
        }
        new_par[i] <- min(upper[i], max(lower[i], new_par[i]))
      }
    }
    return(new_par)
  }
  
  eval_fn_alpha <- function(sol) {
    res <- eval_plan_O3(sol, forecasts, week_id, max_sales_units,
                        verbose = FALSE, max_J = max_J, max_X = max_X,
                        alpha = alpha)
    return(res)
  }
  
  set.seed(123)
  s0 <- build_s0()
  
  hc <- hclimbing(par = s0, fn = eval_fn_alpha, change = change_fn_mixed,
                  lower = lower, upper = upper, type = "max",
                  control = list(maxit = max_iter, REPORT = 0, digits = 2))
  
  comps <- eval_plan_O3(hc$sol, forecasts, week_id, max_sales_units,
                        verbose = FALSE, max_J = max_J, max_X = max_X,
                        return_components = TRUE, alpha = alpha)
  
  return(list(solution = hc$sol, 
              profit = comps$total_profit,
              hr = comps$total_hr,
              sales_units = comps$total_sales_units))
}

# SANN O1
run_sann_O1 <- function(week_id, max_iter = 5000, initial_temp = 5) {
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J, 28), rep(MAX_X, 28), rep(MAX_PR, 28))
  
  sann_gr <- function(par) {
    hchange(par, lower = lower, upper = upper, operator = "*",
            dist = rnorm, mean = 1, sd = 0.05, round = FALSE)
  }
  
  eval_fn <- function(sol) {
    -eval_plan_O1(sol, forecasts = forecasts, week_id = week_id, verbose = FALSE)
  }
  
  set.seed(122)
  s0 <- runif(84, min = lower, max = upper)
  
  control_sann <- list(maxit = max_iter, temp = initial_temp, trace = FALSE, tmax = 30)
  
  sann_result <- optim(par = s0, fn = eval_fn, gr = sann_gr,
                       method = "SANN", control = control_sann)
  
  best_profit <- -sann_result$value
  return(list(solution = sann_result$par, profit = best_profit))
}

# SANN O2
run_sann_O2 <- function(week_id, max_iter = 5000, initial_temp = 50) {
  max_J <- 50
  max_X <- 30
  max_PR <- 0.30
  max_sales_units <- 10000
  
  lower <- rep(0, 84)
  upper <- c(rep(max_J, 28), rep(max_X, 28), rep(max_PR, 28))
  
  sann_gr <- function(par) {
    hchange(par, lower = lower, upper = upper, operator = "*",
            dist = rnorm, mean = 1, sd = 0.05, round = FALSE)
  }
  
  eval_fn_sann <- function(sol) {
    -eval_plan_O2(sol, forecasts = forecasts, week_id = week_id, 
                  max_sales_units = max_sales_units, verbose = FALSE)
  }
  
  set.seed(122)
  s0 <- runif(84, min = lower, max = upper)
  
  control_sann <- list(maxit = max_iter, temp = initial_temp, trace = FALSE, tmax = 30)
  
  sann_result <- optim(par = s0, fn = eval_fn_sann, gr = sann_gr,
                       method = "SANN", control = control_sann)
  
  best_profit <- -sann_result$value
  sol_repaired <- repair_solution_inplace(sann_result$par, forecasts, week_id, max_sales_units)
  final_profit <- eval_plan_O2(sol_repaired, forecasts, week_id, max_sales_units, verbose = FALSE)
  
  cat(sprintf("\n=== SANN O2 Semana %d ===\n", week_id))
  cat(sprintf("Profit: $ %.2f\n", final_profit))
  
  return(list(solution = sol_repaired, profit = final_profit))
}

# SANN O3
run_sann_O3 <- function(week_id, max_iter = 5000, initial_temp = 50, alpha = 0.1) {
  max_J <- 50
  max_X <- 30
  max_PR <- 0.30
  max_sales_units <- 10000
  
  lower <- rep(0, 84)
  upper <- c(rep(max_J, 28), rep(max_X, 28), rep(max_PR, 28))
  
  sann_gr <- function(par) {
    hchange(par, lower = lower, upper = upper, operator = "*",
            dist = rnorm, mean = 1, sd = 0.05, round = FALSE)
  }
  
  eval_fn_sann <- function(sol) {
    -eval_plan_O3(sol, forecasts = forecasts, week_id = week_id,
                  max_sales_units = max_sales_units, verbose = FALSE,
                  alpha = alpha)
  }
  
  set.seed(123)
  s0 <- runif(84, min = lower, max = upper)
  
  control_sann <- list(maxit = max_iter, temp = initial_temp, trace = FALSE, tmax = 20)
  
  sann_result <- optim(par = s0, fn = eval_fn_sann, gr = sann_gr,
                       method = "SANN", control = control_sann)
  
  comps <- eval_plan_O3(sann_result$par, forecasts, week_id, max_sales_units,
                        verbose = FALSE, return_components = TRUE, alpha = alpha)
  
  sol_repaired <- repair_solution_inplace(sann_result$par, forecasts, week_id, max_sales_units)
  comps_repaired <- eval_plan_O3(sol_repaired, forecasts, week_id, max_sales_units,
                                 verbose = FALSE, return_components = TRUE, alpha = alpha)
  
  if(comps_repaired$total_profit > comps$total_profit) {
    comps <- comps_repaired
    sann_result$par <- sol_repaired
  }
  
  cat(sprintf("\n=== SANN O3 Semana %d ===\n", week_id))
  cat(sprintf("Profit: $ %.2f\n", comps$total_profit))
  cat(sprintf("HR total: %d\n", comps$total_hr))
  cat(sprintf("Sales units: %d / %d\n", comps$total_sales_units, max_sales_units))
  
  return(list(solution = sann_result$par, 
              profit = comps$total_profit,
              hr = comps$total_hr,
              sales_units = comps$total_sales_units))
}

# ============================================================================
# GA para O1 - BASEADO NO 21_otimizacao_GA_O1.R (analise_modelos)
# ============================================================================

run_ga_O1 <- function(week_id, popSize = 100, maxiter = 100, 
                      pcrossover = 0.8, pmutation = 0.1, elitism = 10) {
  
  # LIMITES DO GA O1 (como no script)
  MAX_J_GA <- 20
  MAX_X_GA <- 15
  MAX_PR_GA <- 0.30
  
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J_GA, 28), rep(MAX_X_GA, 28), rep(MAX_PR_GA, 28))
  
  # Função de fitness (como no script 21_otimizacao_GA_O1.R)
  fitness_fn <- function(sol) {
    if(is.matrix(sol)) sol <- as.numeric(sol)
    dim(sol) <- c(4, 7, 3)
    
    # Avaliar lucro
    profit <- eval_plan_O1(sol, forecasts = forecasts, week_id = week_id, verbose = FALSE)
    
    # Garantir que não é NA ou infinito
    if(is.na(profit) || !is.finite(profit)) {
      return(-1e9)
    }
    return(profit)
  }
  
  # Executar GA (exatamente como no script, com monitor = TRUE para ver progresso)
  set.seed(123)
  
  cat("\n", paste(rep("=", 70), collapse=""), "\n")
  cat("🎯 ALGORITMO GENÉTICO - O1 (MAXIMIZAR LUCRO)\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("\nParâmetros:\n")
  cat("   Dimensão:", length(lower), "variáveis\n")
  cat("   População:", popSize, "\n")
  cat("   Gerações:", maxiter, "\n")
  cat("   Semana:", week_id, "\n\n")
  
  ga_result <- ga(
    type = "real-valued",
    fitness = fitness_fn,
    lower = lower,
    upper = upper,
    popSize = popSize,
    maxiter = maxiter,
    pcrossover = pcrossover,
    pmutation = pmutation,
    elitism = elitism,
    run = 50,
    monitor = TRUE,   # Monitor = TRUE para ver progresso (como no script)
    seed = 123
  )
  
  # Extrair resultados
  if(is.matrix(ga_result@solution)) {
    best_solution <- ga_result@solution[1, ]
  } else {
    best_solution <- ga_result@solution
  }
  best_profit <- ga_result@fitnessValue
  
  cat("\n", paste(rep("=", 70), collapse=""), "\n")
  cat("📊 RESULTADOS DO ALGORITMO GENÉTICO - O1\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("\n💰 Melhor lucro encontrado: $", round(best_profit, 2), "\n")
  
  return(list(solution = best_solution, profit = best_profit))
}

# ============================================================================
# GA para O2 - BASEADO NO 27_otimizacao_GA_O2_final.R
# ============================================================================

run_ga_O2 <- function(week_id, 
                      popSize = 150, 
                      maxiter = 150, 
                      pcrossover = 0.8, 
                      pmutation = 0.15, 
                      elitism = 30,
                      run = 80) {
  
  # LIMITES DO GA O2 (como no script)
  MAX_J_GA <- 20
  MAX_X_GA <- 15
  MAX_PR_GA <- 0.30
  MAX_UNIDADES <- 10000
  
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J_GA, 28), rep(MAX_X_GA, 28), rep(MAX_PR_GA, 28))
  
  # Função para calcular RH
  calcular_RH <- function(sol) {
    if(is.matrix(sol)) sol <- as.numeric(sol)
    dim(sol) <- c(4, 7, 3)
    rh <- 0
    for(i in 1:4) {
      for(d in 1:7) {
        rh <- rh + max(0, round(sol[i, d, 1]))  # J
        rh <- rh + max(0, round(sol[i, d, 2]))  # X
      }
    }
    return(rh)
  }
  
  # Função de fitness com repair
  fitness_fn <- function(sol) {
    if(is.matrix(sol)) sol <- as.numeric(sol)
    
    # Aplicar repair
    sol_repaired <- repair_solution_inplace(sol, forecasts, week_id, MAX_UNIDADES)
    
    # Calcular lucro
    profit <- eval_plan_O2(sol_repaired, forecasts, week_id, MAX_UNIDADES, verbose = FALSE)
    
    return(profit)  # Maximiza
  }
  
  # População inicial (baixo RH como no script)
  set.seed(123)
  populacao_inicial <- matrix(NA, nrow = popSize, ncol = 84)
  for(i in 1:popSize) {
    for(j in 1:28) {
      populacao_inicial[i, j] <- runif(1, 0, 5)      # J ≤ 5
      populacao_inicial[i, j+28] <- runif(1, 0, 3)   # X ≤ 3
      populacao_inicial[i, j+56] <- runif(1, 0, 0.15) # PR baixo
    }
  }
  
  # Executar GA
  cat("\n", paste(rep("=", 70), collapse=""), "\n")
  cat("🎯 GA O2 - Semana", week_id, "\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("\nParâmetros:\n")
  cat("   População:", popSize, "\n")
  cat("   Gerações:", maxiter, "\n")
  cat("   Crossover:", pcrossover, "\n")
  cat("   Mutação:", pmutation, "\n")
  cat("   Elitismo:", elitism, "\n")
  cat("   Run (paragem):", run, "\n\n")
  
  ga_result <- ga(
    type = "real-valued",
    fitness = fitness_fn,
    lower = lower,
    upper = upper,
    popSize = popSize,
    maxiter = maxiter,
    pcrossover = pcrossover,
    pmutation = pmutation,
    elitism = elitism,
    run = run,
    monitor = TRUE,  # TRUE para ver progresso
    seed = 123,
    suggestions = populacao_inicial
  )
  
  # Melhor solução
  best_solution <- ga_result@solution[1, ]
  best_profit <- ga_result@fitnessValue
  
  # Aplicar repair final à melhor solução
  best_solution_repaired <- repair_solution_inplace(best_solution, forecasts, week_id, MAX_UNIDADES)
  final_profit <- eval_plan_O2(best_solution_repaired, forecasts, week_id, MAX_UNIDADES, verbose = FALSE)
  
  # Calcular RH e unidades finais
  rh_final <- calcular_RH(best_solution_repaired)
  
  # Calcular unidades finais
  calcular_unidades <- function(sol) {
    dim(sol) <- c(4, 7, 3)
    store_names <- c("baltimore", "lancaster", "philadelphia", "richmond")
    F_J <- c(1.00, 1.05, 1.10, 1.15)
    F_X <- c(1.15, 1.20, 1.15, 1.25)
    total_units <- 0
    for(s in 1:4) {
      store <- store_names[s]
      fc_store <- forecasts[forecasts$Store == store & forecasts$Week_ID == week_id, ]
      for(d in 1:7) {
        J <- max(0, round(sol[s, d, 1]))
        X <- max(0, round(sol[s, d, 2]))
        PR <- min(0.30, max(0, sol[s, d, 3]))
        C_pred <- fc_store$Forecast[fc_store$Day == d]
        max_assisted <- 7 * X + 6 * J
        A <- min(max_assisted, C_pred)
        n_X <- min(X * 7, A)
        n_J <- A - n_X
        if(n_X > 0) {
          U_X <- round(F_X[s] * 10 / log(2 - PR))
          total_units <- total_units + n_X * U_X
        }
        if(n_J > 0) {
          U_J <- round(F_J[s] * 10 / log(2 - PR))
          total_units <- total_units + n_J * U_J
        }
      }
    }
    return(total_units)
  }
  
  unidades_final <- calcular_unidades(best_solution_repaired)
  
  cat("\n", paste(rep("=", 70), collapse=""), "\n")
  cat("📊 RESULTADO GA O2\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("\n💰 Lucro: $", round(final_profit, 2), "\n")
  cat("👥 RH (J+X):", rh_final, "funcionários\n")
  cat("📦 Unidades:", unidades_final, "/", MAX_UNIDADES, "\n")
  
  if(unidades_final <= MAX_UNIDADES) {
    cat("✅ Restrição de", MAX_UNIDADES, "unidades RESPECTADA!\n")
  } else {
    cat("❌ Restrição de", MAX_UNIDADES, "unidades VIOLADA!\n")
  }
  
  return(list(solution = best_solution_repaired, 
              profit = final_profit,
              hr = rh_final,
              sales_units = unidades_final))
}

# ============================================================================
# GA para O3 - NSGA-II (MULTIOBJETIVO) - CORRIGIDO
# ============================================================================

run_ga_O3 <- function(week_id, 
                      popSize = 100, 
                      generations = 100, 
                      crossover_prob = 0.8, 
                      mutation_prob = 0.2,
                      alpha = 0.1) {
  
  # Verificar se pacote mco está instalado
  if(!require(mco, quietly = TRUE)) {
    install.packages("mco")
    library(mco)
  }
  
  # LIMITES DO GA O3
  MAX_J_GA <- 20
  MAX_X_GA <- 15
  MAX_PR_GA <- 0.30
  MAX_UNIDADES <- 10000
  
  lower <- rep(0, 84)
  upper <- c(rep(MAX_J_GA, 28), rep(MAX_X_GA, 28), rep(MAX_PR_GA, 28))
  
  # Função multiobjetivo (NSGA-II minimiza ambos)
  multiobjetivo <- function(sol) {
    if(is.matrix(sol)) sol <- as.numeric(sol)
    
    componentes <- eval_plan_O3(sol, forecasts, week_id, MAX_UNIDADES, 
                                verbose = FALSE, 
                                return_components = TRUE,
                                alpha = alpha)
    
    rh <- componentes$total_hr
    profit <- componentes$total_profit
    
    # NSGA-II MINIMIZA ambos os objetivos
    obj1 <- rh                    # minimizar RH
    obj2 <- -profit               # minimizar = maximizar lucro
    
    return(c(obj1, obj2))
  }
  
  # População inicial
  set.seed(123)
  populacao_inicial <- matrix(NA, nrow = popSize, ncol = 84)
  for(i in 1:popSize) {
    for(j in 1:28) {
      populacao_inicial[i, j] <- runif(1, 0, 8)      # J ≤ 8
      populacao_inicial[i, j+28] <- runif(1, 0, 5)   # X ≤ 5
      populacao_inicial[i, j+56] <- runif(1, 0, 0.15) # PR baixo
    }
  }
  
  # Executar NSGA-II
  cat("\n", paste(rep("=", 70), collapse=""), "\n")
  cat("🎯 OTIMIZAÇÃO O3 - NSGA-II (MULTIOBJETIVO)\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("   Objetivo 1: MINIMIZAR RH\n")
  cat("   Objetivo 2: MAXIMIZAR lucro\n")
  cat("\nParâmetros:\n")
  cat("   População:", popSize, "\n")
  cat("   Gerações:", generations, "\n")
  cat("   Alpha:", alpha, "\n\n")
  
  resultado <- nsga2(
    fn = multiobjetivo,
    idim = 84,
    odim = 2,
    lower.bounds = lower,
    upper.bounds = upper,
    popsize = popSize,
    generations = generations,
    cprob = crossover_prob,
    mprob = mutation_prob,
    vectorized = FALSE
  )
  
  # Processar fronteira de Pareto
  pareto <- resultado$pareto.optimal
  objetivos_pareto <- resultado$value[pareto, ]
  
  if(length(pareto) > 0 && sum(pareto) > 0) {
    rh_pareto <- objetivos_pareto[, 1]
    lucros_pareto <- -objetivos_pareto[, 2]
    
    df_pareto <- data.frame(
      RH = round(rh_pareto, 0),
      Lucro = round(lucros_pareto, 2),
      idx = which(pareto)  # guardar índice original
    )
    
    # Remover duplicados e ordenar
    df_pareto <- df_pareto[order(df_pareto$RH), ]
    df_pareto <- df_pareto[!duplicated(df_pareto$RH), ]
    df_pareto <- df_pareto[df_pareto$Lucro > 0, ]
    
    # Calcular função objetivo: rh - alpha * lucro
    df_pareto$F_objetivo <- df_pareto$RH - alpha * df_pareto$Lucro
    
    # ORDENAR por F_objetivo (menor = melhor)
    df_pareto <- df_pareto[order(df_pareto$F_objetivo), ]
    
    # A MELHOR SOLUÇÃO É A PRIMEIRA (menor F)
    melhor_linha <- df_pareto[1, ]
    melhor_rh <- melhor_linha$RH
    melhor_lucro <- melhor_linha$Lucro
    melhor_idx_original <- melhor_linha$idx
    
    # Encontrar o índice na lista de soluções Pareto
    indices_pareto <- which(pareto)
    melhor_pos <- which(indices_pareto == melhor_idx_original)
    
    # Obter a solução correspondente
    melhor_solucao_raw <- resultado$par[pareto, ][melhor_pos, ]
    
    # Aplicar repair final
    melhor_solucao <- repair_solution_inplace(melhor_solucao_raw, forecasts, week_id, MAX_UNIDADES)
    
    # Calcular componentes finais
    comps_final <- eval_plan_O3(melhor_solucao, forecasts, week_id, MAX_UNIDADES,
                                verbose = FALSE, return_components = TRUE, alpha = alpha)
    
    # Mostrar fronteira de Pareto (ordenada por F)
    cat("\n", paste(rep("=", 70), collapse=""), "\n")
    cat("📊 FRONTEIRA DE PARETO (ordenada por F = RH - alpha*Lucro)\n")
    cat(paste(rep("=", 70), collapse=""), "\n")
    cat(sprintf("   %3s | %8s | %15s | %s\n", "RH", "Lucro", "F", "Melhor?"))
    cat("   ", paste(rep("-", 50), collapse=""), "\n")
    
    for(i in 1:nrow(df_pareto)) {
      is_best <- if(i == 1) "🏆 SIM" else ""
      cat(sprintf("   %3d | $%6.0f | %12.2f | %s\n", 
                  df_pareto$RH[i], df_pareto$Lucro[i], df_pareto$F_objetivo[i], is_best))
    }
    
    cat("\n", paste(rep("=", 70), collapse=""), "\n")
    cat("🏆 MELHOR SOLUÇÃO SELECIONADA\n")
    cat(paste(rep("=", 70), collapse=""), "\n")
    cat(sprintf("   👥 RH = %d funcionários\n", comps_final$total_hr))
    cat(sprintf("   💰 Lucro = $ %.2f\n", comps_final$total_profit))
    cat(sprintf("   📦 Unidades = %d / %d\n", comps_final$total_sales_units, MAX_UNIDADES))
    cat(sprintf("   📈 F = %.2f\n", comps_final$total_hr - alpha * comps_final$total_profit))
    
    return(list(solution = melhor_solucao,
                profit = comps_final$total_profit,
                hr = comps_final$total_hr,
                sales_units = comps_final$total_sales_units,
                pareto_front = df_pareto))
    
  } else {
    cat("\n❌ Nenhuma solução de Pareto encontrada!\n")
    return(list(solution = NULL, profit = -Inf, hr = Inf, sales_units = 0))
  }
}


# ============================================================================
# PROCESSAR SOLUÇÃO PARA TABELA
# ============================================================================

process_solution <- function(sol, week_id, objective, selected_store = NULL) {
  if(is.null(sol)) {
    return(list(details = data.frame(), total_profit = 0, total_sales_units = 0, total_hr = 0))
  }
  
  dim(sol) <- c(4, 7, 3)
  
  W_s <- c(700, 730, 760, 800)
  F_J <- c(1.00, 1.05, 1.10, 1.15)
  F_X <- c(1.15, 1.20, 1.15, 1.25)
  hr_cost_J_weekday <- 60
  hr_cost_J_weekend <- 70
  hr_cost_X_weekday <- 80
  hr_cost_X_weekend <- 95
  
  results <- data.frame()
  total_profit <- 0
  total_sales_units <- 0
  total_hr <- 0
  
  if(objective == "O1" && !is.null(selected_store)) {
    stores_to_show <- which(store_names_print == selected_store)
  } else {
    stores_to_show <- 1:4
  }
  
  for(s in stores_to_show) {
    store <- store_names[s]
    fc_store <- forecasts[forecasts$Store == store & forecasts$Week_ID == week_id, ]
    if(nrow(fc_store) != 7) next
    
    for(d in 1:7) {
      J <- max(0, round(sol[s, d, 1]))
      X <- max(0, round(sol[s, d, 2]))
      PR <- min(0.30, max(0, sol[s, d, 3]))
      
      C_pred <- fc_store$Forecast[fc_store$Day == d]
      max_assisted <- 7 * X + 6 * J
      A <- min(max_assisted, C_pred)
      
      is_weekend <- (d %in% c(1, 7))
      cost_J <- ifelse(is_weekend, hr_cost_J_weekend, hr_cost_J_weekday)
      cost_X <- ifelse(is_weekend, hr_cost_X_weekend, hr_cost_X_weekday)
      daily_cost_hr <- J * cost_J + X * cost_X
      
      n_assisted_by_X <- min(X * 7, A)
      n_assisted_by_J <- A - n_assisted_by_X
      
      daily_sales_units <- 0
      daily_revenue_raw <- 0
      
      if(n_assisted_by_X > 0) {
        U_X <- round(F_X[s] * 10 / log(2 - PR))
        P_X_raw <- U_X * (1 - PR) * 1.07
        daily_sales_units <- daily_sales_units + n_assisted_by_X * U_X
        daily_revenue_raw <- daily_revenue_raw + n_assisted_by_X * P_X_raw
      }
      if(n_assisted_by_J > 0) {
        U_J <- round(F_J[s] * 10 / log(2 - PR))
        P_J_raw <- U_J * (1 - PR) * 1.07
        daily_sales_units <- daily_sales_units + n_assisted_by_J * U_J
        daily_revenue_raw <- daily_revenue_raw + n_assisted_by_J * P_J_raw
      }
      
      daily_revenue <- round(daily_revenue_raw)
      daily_net_profit <- daily_revenue - daily_cost_hr
      
      total_sales_units <- total_sales_units + daily_sales_units
      total_hr <- total_hr + J + X
      
      day_name <- c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat")[d]
      
      results <- rbind(results, data.frame(
        Store = store_names_print[s],
        Day = day_name,
        J = J,
        X = X,
        PR = sprintf("%.1f%%", PR * 100),
        Customers = round(C_pred),
        Assisted = round(A),
        Sold_Units = round(daily_sales_units),
        Revenue = sprintf("$%.0f", daily_revenue),
        HR_Cost = sprintf("$%.0f", daily_cost_hr),
        Daily_Profit = sprintf("$%.0f", daily_net_profit)
      ))
      
      total_profit <- total_profit + daily_net_profit
    }
    total_profit <- total_profit - W_s[s]
  }
  
  return(list(details = results, 
              total_profit = total_profit,
              total_sales_units = total_sales_units,
              total_hr = total_hr))
}

# ============================================================================
# UI
# ============================================================================

ui <- fluidPage(
  theme = shinytheme("flatly"),
  titlePanel(
    div(style = "text-align: center;",
        h1("Intelligent Decision Support System"),
        h4("USA Stores - Forecasting & Optimization")
    )
  ),
  
  sidebarLayout(
    sidebarPanel(
      width = 3,
      
      conditionalPanel(
        condition = "input.objective == 'O1'",
        selectInput("store", "Store",
                    choices = store_names_print,
                    selected = "Baltimore")
      ),
      
      selectInput("week", "Week",
                  choices = semanas_disponiveis,
                  selected = 20),
      
      radioButtons("objective", "Optimization Goal",
                   choices = c(
                     "O1: Maximize Profit (per store)" = "O1",
                     "O2: Maximize Profit (all stores, ≤10,000 units)" = "O2",
                     "O3: Maximize Profit + Minimize HR (all stores)" = "O3"
                   ),
                   selected = "O1"),
      
      radioButtons("method", "Optimization Method",
                   choices = c(
                     "Hill Climbing (HC)" = "HC",
                     "Simulated Annealing (SANN)" = "SANN",
                     "Genetic Algorithm (GA)" = "GA"
                   ),
                   selected = "HC"),
      
      conditionalPanel(
        condition = "input.objective == 'O1'",
        numericInput("max_iter_O1", "Max Iterations", value = 5000, min = 500, max = 20000, step = 500)
      ),
      conditionalPanel(
        condition = "input.objective == 'O2'",
        numericInput("max_iter_O2", "Max Iterations", value = 10000, min = 1000, max = 50000, step = 1000)
      ),
      conditionalPanel(
        condition = "input.objective == 'O3'",
        numericInput("max_iter_O3", "Max Iterations", value = 8000, min = 1000, max = 50000, step = 1000)
      ),
      
      conditionalPanel(
        condition = "input.method == 'GA' && input.objective == 'O1'",
        numericInput("ga_popSize", "Population Size", value = 100, min = 20, max = 200, step = 10),
        numericInput("ga_maxiter", "Generations", value = 100, min = 50, max = 500, step = 10),
        numericInput("ga_pcrossover", "Crossover Rate", value = 0.8, min = 0.5, max = 0.95, step = 0.05),
        numericInput("ga_pmutation", "Mutation Rate", value = 0.1, min = 0.01, max = 0.3, step = 0.01),
        numericInput("ga_elitism", "Elitism", value = 10, min = 5, max = 50, step = 5)
      ),
      
      conditionalPanel(
        condition = "input.method == 'GA' && input.objective == 'O2'",
        numericInput("ga_popSize_O2", "Population Size", value = 150, min = 50, max = 300, step = 10),
        numericInput("ga_maxiter_O2", "Generations", value = 150, min = 50, max = 500, step = 10),
        numericInput("ga_pcrossover_O2", "Crossover Rate", value = 0.8, min = 0.5, max = 0.95, step = 0.05),
        numericInput("ga_pmutation_O2", "Mutation Rate", value = 0.15, min = 0.01, max = 0.3, step = 0.01),
        numericInput("ga_elitism_O2", "Elitism", value = 30, min = 5, max = 50, step = 5),
        numericInput("ga_run_O2", "Run (stopping)", value = 80, min = 20, max = 200, step = 10)
      ),
      
      conditionalPanel(
        condition = "input.method == 'GA' && input.objective == 'O3'",
        numericInput("ga_popSize_O3", "Population Size", value = 100, min = 50, max = 300, step = 10),
        numericInput("ga_generations_O3", "Generations", value = 100, min = 50, max = 500, step = 10),
        numericInput("ga_crossover_O3", "Crossover Rate", value = 0.8, min = 0.5, max = 0.95, step = 0.05),
        numericInput("ga_mutation_O3", "Mutation Rate", value = 0.2, min = 0.05, max = 0.4, step = 0.05),
        numericInput("ga_alpha_O3", "Alpha (HR weight)", value = 0.1, min = 0.01, max = 0.5, step = 0.01)
      ),
      
      actionButton("run", "Generate Optimized Plan",
                   class = "btn-primary btn-lg",
                   style = "width: 100%; margin-top: 20px;"),
      
      hr(),
      div(style = "font-size: 12px; color: gray;",
          p("HC/SANN: Max J=50, Max X=30 | GA: Max J=20, Max X=15"),
          p("All methods use same objective functions")
      )
    ),
    
    mainPanel(
      width = 9,
      tabsetPanel(
        tabPanel("Forecasts",
                 br(),
                 h4("Customer Forecasts for Selected Week"),
                 DTOutput("forecast_table"),
                 br(),
                 h4("Forecast Visualization"),
                 plotlyOutput("forecast_plot", height = "400px")
        ),
        
        tabPanel("Optimized Plan",
                 br(),
                 div(style = "background-color: #e8f4f8; padding: 15px; border-radius: 8px; margin-bottom: 20px;",
                     h4("Summary Results"),
                     fluidRow(
                       column(4, h5("Total Profit:"), h3(textOutput("total_profit"))),
                       column(4, h5("Total Sold Units:"), h3(textOutput("total_units"))),
                       column(4, h5("Total HR (J+X):"), h3(textOutput("total_hr")))
                     )
                 ),
                 br(),
                 h4("Detailed Weekly Plan"),
                 DTOutput("plan_table"),
                 br(),
                 h4("Profit by Store"),
                 plotlyOutput("profit_chart", height = "400px")
        ),
        
        tabPanel("About",
                 br(),
                 h4("Decision Support System for USA Stores"),
                 p("This system integrates:"),
                 tags$ul(
                   tags$li("Customer forecasts for 4 stores (104 weeks)"),
                   tags$li("Hill Climbing / SANN / GA optimization for 3 objectives"),
                   tags$li("Parameters: HC/SANN (J=50, X=30) | GA (J=20, X=15)")
                 )
        )
      )
    )
  )
)

# ============================================================================
# SERVER
# ============================================================================

server <- function(input, output, session) {
  
  opt_results <- reactiveVal(NULL)
  is_running <- reactiveVal(FALSE)
  
  # Mostrar previsões
  output$forecast_table <- renderDT({
    store_sel <- tolower(input$store)
    fc_data <- forecasts[forecasts$Store == store_sel & forecasts$Week_ID == input$week, ]
    fc_data <- fc_data[order(fc_data$Day), ]
    
    day_names <- c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")
    
    fc_display <- data.frame(
      Day = day_names[fc_data$Day],
      Forecast_Customers = round(fc_data$Forecast, 0)
    )
    
    datatable(fc_display, 
              options = list(pageLength = 7, dom = 't'),
              rownames = FALSE) %>%
      formatStyle("Forecast_Customers",
                  background = styleColorBar(fc_data$Forecast, "lightblue"),
                  backgroundSize = '100% 90%',
                  backgroundRepeat = 'no-repeat')
  })
  
  output$forecast_plot <- renderPlotly({
    store_sel <- tolower(input$store)
    fc_data <- forecasts[forecasts$Store == store_sel & forecasts$Week_ID == input$week, ]
    fc_data <- fc_data[order(fc_data$Day), ]  # Garantir ordem correta
    
    day_names <- c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat")
    
    # Nomes dos dias na ordem correta (1=Domingo, 2=Segunda, ... conforme teu sistema)
    # No teu código, d=1 é Domingo, d=7 é Sábado
    day_names <- c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat")
    
    plot_ly() %>%
      add_trace(x = day_names, y = fc_data$Forecast,
                type = 'scatter', mode = 'lines+markers',
                name = 'Forecast',
                line = list(color = 'steelblue', width = 3),
                marker = list(size = 10, color = 'steelblue')) %>%
      layout(title = paste("Customer Forecast -", input$store),
             xaxis = list(title = "Day", categoryorder = "array",
                          categoryarray = day_names),
             yaxis = list(title = "Number of Customers"))
  })
  
  # Executar otimização
  observeEvent(input$run, {
    if(is_running()) {
      showNotification("Optimization already running...", type = "warning")
      return()
    }
    
    is_running(TRUE)
    
    max_iter <- switch(input$objective,
                       O1 = input$max_iter_O1,
                       O2 = input$max_iter_O2,
                       O3 = input$max_iter_O3)
    
    showNotification(paste("Running", input$method, "-", input$objective, "optimization for", 
                           ifelse(input$objective == "O1", input$store, "all stores"),
                           "- this may take a few minutes..."), 
                     type = "default", duration = 5)
    
    withProgress(message = paste(input$method, '-', input$objective, 'optimizing...'), value = 0, {
      
      result <- NULL
      
      if(input$objective == "O1") {
        incProgress(0.2, detail = "Setting up O1 optimization")
        if(input$method == "HC") {
          result <- run_optimization_O1(as.numeric(input$week), max_iter)
        } else if(input$method == "SANN") {
          result <- run_sann_O1(as.numeric(input$week), max_iter, initial_temp = 5)
        } else {  # GA
          result <- run_ga_O1(as.numeric(input$week),
                              popSize = input$ga_popSize,
                              maxiter = input$ga_maxiter,
                              pcrossover = input$ga_pcrossover,
                              pmutation = input$ga_pmutation,
                              elitism = input$ga_elitism)
        }
      } else if(input$objective == "O2") {
        incProgress(0.2, detail = "Setting up O2 optimization (this may take longer)")
        if(input$method == "HC") {
          result <- run_optimization_O2(as.numeric(input$week), max_iter)
        } else if(input$method == "SANN") {
          result <- run_sann_O2(as.numeric(input$week), max_iter, initial_temp = 50)
        } else {  # GA
          result <- run_ga_O2(as.numeric(input$week),
                              popSize = input$ga_popSize_O2,
                              maxiter = input$ga_maxiter_O2,
                              pcrossover = input$ga_pcrossover_O2,
                              pmutation = input$ga_pmutation_O2,
                              elitism = input$ga_elitism_O2,
                              run = input$ga_run_O2)
        }
      } else {  # O3
        incProgress(0.2, detail = "Setting up O3 optimization")
        if(input$method == "HC") {
          result <- run_optimization_O3(as.numeric(input$week), max_iter, alpha = 0.1)
        } else if(input$method == "SANN") {
          result <- run_sann_O3(as.numeric(input$week), max_iter, initial_temp = 50, alpha = 0.1)
        } else {  # GA (NSGA-II)
          result <- run_ga_O3(as.numeric(input$week),
                              popSize = input$ga_popSize_O3,
                              generations = input$ga_generations_O3,
                              crossover_prob = input$ga_crossover_O3,
                              mutation_prob = input$ga_mutation_O3,
                              alpha = input$ga_alpha_O3)
        }
      }
      
      incProgress(0.7, detail = "Processing results")
      
      # Para O3, usar valores diretos da otimização
      if(input$objective == "O3") {
        processed <- process_solution(result$solution, as.numeric(input$week), 
                                      input$objective, input$store)
        
        opt_results(list(
          details = processed$details,
          total_profit = result$profit,
          total_sales_units = result$sales_units,
          total_hr = result$hr
        ))
      } else {
        processed <- process_solution(result$solution, as.numeric(input$week), 
                                      input$objective, input$store)
        
        opt_results(list(
          details = processed$details,
          total_profit = processed$total_profit,
          total_sales_units = processed$total_sales_units,
          total_hr = processed$total_hr
        ))
      }
      
      incProgress(1, detail = "Complete!")
    })
    
    showNotification(paste("Optimization completed! Profit: $", 
                           round(opt_results()$total_profit, 2)), 
                     type = "default", duration = 3)
    
    is_running(FALSE)
  })
  
  output$total_profit <- renderText({
    res <- opt_results()
    if(is.null(res)) return("Click 'Generate'")
    sprintf("$ %.2f", res$total_profit)
  })
  
  output$total_units <- renderText({
    res <- opt_results()
    if(is.null(res)) return("Click 'Generate'")
    format(round(res$total_sales_units), big.mark = ",")
  })
  
  output$total_hr <- renderText({
    res <- opt_results()
    if(is.null(res)) return("Click 'Generate'")
    res$total_hr
  })
  
  output$plan_table <- renderDT({
    res <- opt_results()
    if(is.null(res) || nrow(res$details) == 0) {
      return(datatable(data.frame(Message = "Click 'Generate Optimized Plan'")))
    }
    
    datatable(res$details,
              options = list(pageLength = 28, scrollX = TRUE),
              rownames = FALSE) %>%
      formatStyle("Daily_Profit",
                  color = styleInterval(0, c("red", "darkgreen")))
  })
  
  output$profit_chart <- renderPlotly({
    res <- opt_results()
    if(is.null(res) || nrow(res$details) == 0) return(NULL)
    
    profit_by_store <- res$details %>%
      group_by(Store) %>%
      summarise(Profit = sum(as.numeric(gsub("\\$", "", Daily_Profit))))
    
    plot_ly() %>%
      add_trace(x = profit_by_store$Store, y = profit_by_store$Profit,
                type = 'bar', name = 'Profit',
                marker = list(color = c('#2ecc71', '#3498db', '#e74c3c', '#9b59b6'))) %>%
      layout(title = "Weekly Profit by Store",
             xaxis = list(title = "Store"),
             yaxis = list(title = "Profit ($)"))
  })
}

shinyApp(ui = ui, server = server)