...
# Maximizar lucro e minimizar número total de HR (J+X)
# Fórmula: lucro - peso * total_HR
################################################################################

eval_plan_O3 <- function(sol, forecasts, week_id, 
  ...
  if(verbose) {
    ...
    cat("Peso HR:", weight_hr, "\n")
    cat("Função objetivo (profit - peso*HR):", round(total_profit - weight_hr * total_hr, 2), "\n")
  }
  
  # O3: Aplicar restrição de unidades (herdada do O2) + minimizar HR
  if(total_sales_units > max_sales_units) {
    if(verbose) cat("\n*** PENALIZAÇÃO: Limite de unidades excedido! ***\n")
    return(-1e9)  # Death penalty
  }
  
  # Multiobjetivo: maximizar profit e minimizar HR
  # Quanto maior o weight_hr, mais importância à minimização de HR
