# 12_otimizacao_O3.R - Otimização O3 (O2 + Minimizar RH)
...
PENALIDADE_RH <- 50    # Penalidade por funcionário (cada um custa $50)
...
# ============================================================
# FUNÇÃO DE OTIMIZAÇÃO PARA O3
# ============================================================

otimizar_O3 <- function(previsoes, datas, loja) {
    ...
    for(J in valores_J) {
      for(X in valores_X) {
        for(PR in valores_PR) {
          ...
          # O3: Penalizar uso de funcionários
          lucro <- lucro - PENALIDADE_RH * (J + X)
  ...
  return(list(
    ...
    total_RH = sum(melhor_J) + sum(melhor_X)
  ))
}
...
# ============================================================
# EXECUTAR OTIMIZAÇÃO O3
# ============================================================

cat("\n", paste(rep("=", 70), collapse=""), "\n")
cat("OTIMIZAÇÃO O3 - ARIMAX (Limite 10k + Minimizar RH)\n")
...
for(loja in names(dados)) {
  ...
  plano <- otimizar_O3(previsoes, datas_futuras, loja)
  
  cat("\n   📊 RESULTADOS O3:\n")
  cat("      Lucro semanal: $", round(plano$lucro_semanal, 2), "\n", sep="")
  cat("      Unidades totais:", format(plano$unidades_totais, big.mark = ","), "\n")
  cat("      Total funcionários:", plano$total_RH, "\n")
  
  resultados_O3[[loja]] <- plano
}
...
saveRDS(resultados_O3, "analise_modelos/resultados_otimizacao_O3.rds")

cat("\n✅ Resultados guardados em 'analise_modelos/resultados_otimizacao_O3.rds'\n")